...
int main() {
  ...
  epoll_event_list = (struct epoll_event *)calloc(config->epoll_num,
    ...
    if (epoll_ready_num == -1 && errno == EINTR) {
      ...
      for (int i = 0; i < epoll_ready_num; i++) {
        ...
        if (tempFd == listenfd) {
          ...
          // 终端打印连接的ip和port
          const char *client_ip = inet_ntop(
              AF_INET, &client_addr.sin_addr.s_addr, buf, sizeof(buf));
          printf("client ip: %s, port: %d\n", client_ip,
                 ntohs(client_addr.sin_port));

          // 加入队列
          task_enqueue(&pthread_pool.queue, clientFd, client_ip,
                       ntohs(client_addr.sin_port));
        }
